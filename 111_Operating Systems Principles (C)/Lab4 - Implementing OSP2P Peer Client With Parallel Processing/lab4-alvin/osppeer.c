...
#define MAXIMUM_FILE_SIZE TASKBUFSIZ*4096	// Maximum file size
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	if (evil_mode) {
		message("In evil mode, attacking.\n");
		// Buffer overrun
		char buff_overrun[2*FILENAMESIZ];
		memset(buff_overrun, 1, 2*FILENAMESIZ);
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	// To calculate download rate
	int j, avg_buff_rate, speed_samples[10];
	...
	while (1) {
		...
		if (ret == TBUF_ERROR) {
			...
			/* End of file */
			break;
		// For endless downloads
		if (t->total_written > MAXIMUM_FILE_SIZE) {
			error("* '%s' exceeds maximum file size.\n", t->disk_filename);
			goto try_again;
		}
		...
		// Block slow peers (non-functioning connections)
		current_sample = (current_sample+1)%10;
		speed_samples[current_sample] = t->total_written - last_buff_size;
		last_buff_size = t->total_written;
		avg_buff_rate = 0;
		for (j=0; j < 10; j++) {
			avg_buff_rate+=speed_samples[j];
		}
		avg_buff_rate = avg_buff_rate / 10;
		if (avg_buff_rate < 32) {
			error("* Non-functioning connection.\n");
			goto try_again;
		}
	}
...
static void task_upload(task_t *t)
{
	...
	if ( strlen(t->filename) > FILENAMESIZ) {
		error("* Peer requested file with name too long.\n");
		goto exit;
	}
	...
	if ( strncmp(current_working_directory, file_path, strlen(current_working_directory)) ) {
	...
	if (evil_mode) {
		...
		while (1) {
			...
			if (ret == TBUF_ERROR) {
				message("* Endless upload attack successful.\n");
				goto exit;
			}
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	pid_t pid;
	// Fork processes for each file for parallel downloads
	for (; argc > 1; argc--, argv++) {
		if ((t = start_download(tracker_task, argv[1]))) {
			...
			// Error Check
			if (pid == -1) {
				error("Failed to fork for download.\n");
			}
			...
			// Don't do anything with parent process
		}
	}
	...
	while ((t = task_listen(listen_task))) {
		...
		// Error check
		if (pid == -1) {
			error("Failed to fork for upload.\n");
		}
		// Child process has PID 0
		else if (pid == 0) {
			task_upload(t);
