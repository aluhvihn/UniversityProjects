...
RC BTreeIndex::insert(int key, const RecordId& rid)
{
	...
	// Find out where to insert new key & record
		...
		rc = insert_key(rid, key, rootPid, ret_key, ret_pid, treeHeight);
	}
  return rc;
}
...
RC BTreeIndex::insert_key( const RecordId& rid, int key, PageId start_pid,  int& ret_key, PageId ret_pid, int curr_height)
{
	...
	if (curr_height != 0) {
		...
		rc = this->insert_key(rid, key, next_pid, ret_key, ret_pid, curr_height-1);
		if(ret_pid != -1){
			...
			if (treeHeight == curr_height && start_pid == rootPid) {
				BTNonLeafNode root;
				rc = root.initializeRoot(rootPid, ret_key, ret_pid);
				rootPid = pf.endPid();
	...
	// pid is a leaf node
		...
		split_node.write(ret_pid, pf);
		if(start_pid == rootPid){
		// If the start_pid is the rootPid and we are a leaf node and splitting
		
			BTNonLeafNode rootNode; 
			rc = rootNode.initializeRoot(rootPid, ret_key, ret_pid);
			rootPid = pf.endPid();
			rc = rootNode.write(rootPid, pf);
			ret_key = -1;
			ret_pid = -1;
		...
		return leafnode.write(start_pid,pf);
	}	
	return rc;
}
...
RC BTreeIndex::locate(int searchKey, IndexCursor& cursor)
{
	...
	RC check;
	if(treeHeight > 0) {
		for (currHeight = 0; currHeight < treeHeight; currHeight++) {
...
RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid)
{
	...
	if( ((cursor.eid+12)/12) >= (tempLeaf.getKeyCount()) ){
